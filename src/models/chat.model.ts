...
export const conversation_model = pgTable("conversations", {
  ...
  creater_id: bigint({ mode: 'number' }).references(() => user_model.id).notNull(), // creater/owner
...
export const conversation_member_model = pgTable("conversation_members", {
  ...
  conversation_id: bigint({ mode: 'number' }).references(() => conversation_model.id).notNull(),
  user_id: bigint({ mode: 'number' }).references(() => user_model.id).notNull(),
...
export const message_model = pgTable("messages", {
  id: bigserial({ mode: "number" }).primaryKey(),
  conversation_id: bigint({ mode: 'number' }).references(() => conversation_model.id).notNull(),
  sender_id: bigint({ mode: 'number' }).references(() => user_model.id).notNull(),
