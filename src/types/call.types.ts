...
const CALL_TYPE_CONSTS = ["audio", "video"] as const;
...
type CallTypeType = (typeof CALL_TYPE_CONSTS)[number];

// <--------------------Custom Parsers for cache------------------------>
const parse_call_cache_row = (
  raw: Record<string, string> | null | undefined,
): ActiveCall | null => {
  // The cache row is written by `serializeActiveCall`, which stores the id as
  // `call_id` only. Requiring `raw.id` here made this parser return null for
  // EVERY cached call, which in turn made the ring-timeout path a silent no-op.
  ...
  return {
    ...
    call_type: (raw.call_type as CallTypeType | undefined) ?? "audio",
